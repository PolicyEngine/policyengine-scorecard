...
def load_pe():
    ...
    idx = {}
    for r in rows:
        idx[(r["run"], r["program"], r["metric"], r["subgroup"], r["geography"])] = r[
            "value"
        ]
    return idx
...
def counterpart(pe, program, metric, subgroup, variant, geo, unit_concept):
    ...
    if program == "snap":
        ...
        if metric == "eligibility_rate":
            return (
                ratio(elig, person_denom(subgroup)),
                "comparable",
    ...
    if program == "ssi":
        ...
        if metric == "eligibility_rate":
            ...
            return (
                ratio(payable, person_denom(denom_sub)),
                "constructed",
                recipe + " ÷ adults",
            )
        if metric == "participation_rate":
    ...
    if program == "tanf":
        ...
        if subgroup not in ({"total"} | AGE_SUBS | DEMO_SUBS):
            return None, "not_computed", None
        payable = g("fullpart_urban6", "tanf", "participant_count", subgroup, geo)
        ...
        if metric == "eligibility_rate":
            return (
                ratio(payable, person_denom(subgroup)),
                "constructed",
                recipe + " ÷ persons",
            )
        if metric == "participation_rate":
    ...
    if program == "eitc":
        ...
        if metric == "eligibility_rate":
            if subgroup != "total":
                return None, "not_computed", None
            return (
                ratio(forced, g("baseline", "_tax_units", "count", "total", geo)),
                "constructed",
                recipe + " ÷ tax units",
            )
    ...
    if program == "housing":
        ...
        if metric == "eligibility_rate":
            return (
                ratio(elig, g("baseline", "_spm_units", "count", "total", geo)),
                "concept_mismatch",
                "eligible spm units ÷ spm units",
            )
        if metric == "participation_rate":
            return (
                ratio(both, elig),
...
def annotation_ids(annotations, row):
    ...
    for a in annotations:
        ...
        if m.get("metrics") is not None and row["metric"] not in m["metrics"]:
            continue
        if m.get("subgroups") is not None and row["subgroup"] not in m["subgroups"]:
            continue
        if m.get("variants") is not None and row["variant"] not in m["variants"]:
            continue
...
CALIBRATION_RELATIONSHIP = {
    ...
    ("spm_poverty", None): (
        "held_out",
        "PERMANENT holdout (doctrine 2026-08-02): no tax-benefit quantity "
        "from a survey — reported or computed — nor anything derived from "
        "such may be a calibration target; SPM poverty derives from survey "
        "tax-benefit measurement. Admin tax-benefit quantities and raw "
        "survey margins (ACS population/structure) remain legitimate "
        "targets. Release gates may fail on held-out regressions.",
    ),
...
def calibration_relationship(program, metric):
    hit = CALIBRATION_RELATIONSHIP.get(
        (program, metric)
    ) or CALIBRATION_RELATIONSHIP.get((program, None))
...
def load_pe_2026():
    ...
    return PE(
        {
            (r["run"], r["program"], r["metric"], r["subgroup"], r["geography"]): r[
                "value"
            ]
            for r in rows
        }
...
def main():
    ...
    for ext in externals:
        ...
        if (
            row["pe_value_2026"] is None
            and pe_value is not None
            and ext["subgroup"] == "total"
            and not ext["variant"]
        ):
            k = (
                ext["program"],
                ext["metric"],
                ext["geography"],
                ext["subgroup"] if ext["program"] == "spm_poverty" else "total",
            )
            ic24 = ic_2024.get(k)
            ic26 = ic_2026.get(k)
            if (
                ic24 is not None
                and ic26 is not None
                and abs(ic24 - pe_value) <= abs(pe_value) * 0.005 + 1e-9
            ):
                row["pe_value_2026"] = ic26
        rel, rel_basis = calibration_relationship(ext["program"], ext["metric"])
        row["calibration_relationship"] = rel
        row["calibration_basis"] = rel_basis
        d = diagnoses.get(
            (ext["program"], ext["metric"], ext["geography"], ext["subgroup"])
        )
        row["diagnosis"] = (
            {
                "queue_id": d["queue_id"],
    ...
    for prog, metric in [
        ("snap", "participation_rate"),
        ("ssi", "participation_rate"),
        ("tanf", "participation_rate"),
        ("wic", "participation_rate"),
