...
US families attach here. The UK side lands via its sibling,
scorecard_db/ingest_campaign_uk.py — it supersedes the staged UK
descriptor files (uk/*.jsonl) with fuller recomputed coverage from the
vendored raw runs (uk_runs/), and takes the two-child pentagon
attachments from them; this module still fails loudly if pointed at the
UK directory directly.
...
# Every day-1 run executed against the certified current-law world
# (CPSP's take-up forcing and the fullpart subgroup slices are
# input-override constructions ON that world) — recorded per issue #13.
