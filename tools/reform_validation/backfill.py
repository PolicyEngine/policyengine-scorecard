...
(microcosm.build.us_runtime.reform_validation.reform_validation_payload) on the
...
versions from the release's release_manifest are installed, and the microcosm
package tree is importable (PYTHONPATH to packages/*/src of a microcosm checkout).
...
def _is_obbba(spec):
    from microcosm.build.us_runtime.reform_validation import _is_obbba_spec
...
def _load_specs():
    from microcosm.build.us_runtime.reform_validation import load_default_reform_specs
...
def run_batch(name, subset, levels_slice):
    ...
    # Stamp BOTH revisions this partial was scored under so merge() can refuse
    # a workdir whose partials span two revisions: the producer (a resume that
    # crossed a microcosm main bump) and the driver (backfill.py is re-baked
    # from the current scorecard checkout on every `modal deploy`, so two
    # respawns can run different driver code under one producer stamp).
    payload["_producer_commit"] = os.environ.get("RV_PRODUCER_COMMIT", "unknown")
    payload["_driver_commit"] = os.environ.get("RV_DRIVER_COMMIT", "unknown")
...
def _backfill_note(manifest: dict, producer_commit: str, h5_sha: str) -> str:
    ...
    return (
        ...
        "(microcosm.build.us_runtime.reform_validation.reform_validation_payload) on "
        ...
        f"Producer run at microcosm {producer_commit}. Computed in small subprocess "
...
def _revision_dirty(commits: set) -> bool:
    """A partial-revision set is unusable if it is empty, mixes revisions, or
    contains a missing/unknown stamp."""
    return not commits or None in commits or "unknown" in commits or len(commits) > 1


def _refuse_if_mixed(
    producer_commits: set, driver_commits: set, part_dir: Path
) -> None:
    """Every partial in a workdir must carry a SINGLE, KNOWN producer and
    driver revision. A resume that crossed a microcosm bump (producer) or
    re-``modal deploy``ed a different scorecard checkout (driver) resumes in
    the same release-keyed workdir and would merge two revisions while
    labelling the artifact with one. Refuse on any missing/unknown OR mixed
    stamp — and PURGE the partials so the next respawn recomputes clean,
    instead of wedging in a silent refuse loop where every tick fails
    identically (blocker 4). Pure stdlib so it is unit-testable off-Modal."""
    if _revision_dirty(producer_commits) or _revision_dirty(driver_commits):
        shutil.rmtree(part_dir, ignore_errors=True)
        raise SystemExit(
            "partials span multiple or unstamped revisions (producer="
            f"{sorted(str(c) for c in producer_commits)}, driver="
            f"{sorted(str(c) for c in driver_commits)}) — purged the workdir "
            "partials; the next run recomputes at a single revision."
        )


def merge(manifest: dict, producer_commit: str, h5_sha: str) -> Path:
    from microcosm.build.us_runtime.reform_validation import write_reform_validation

    wd, _, _, part_dir = _paths()
    names = [n for n, _, _ in _batches(_load_specs())]
    seen, rows, header = set(), [], None
    producer_commits, driver_commits = set(), set()
    for name in names:
        p = json.loads((part_dir / f"{name}.json").read_text())
        producer_commits.add(p.get("_producer_commit"))
        driver_commits.add(p.get("_driver_commit"))
        if header is None:
            header = {
                k: v
                for k, v in p.items()
                if k not in ("reforms", "_producer_commit", "_driver_commit")
            }
        for r in p["reforms"]:
            rid = r.get("id")
