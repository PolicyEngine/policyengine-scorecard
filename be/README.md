...
- 1 `euromod`-labelled row — unemployment benefits — is a **non-simulated uprated EU-SILC survey input** (Table A3.6 marks `bun` Simulated=N, p. 127; `bun_be` is switched off in the baseline, p. 24; the 11,706 for 2023 is the SILC income-year-2021 base of 10,416 uprated) and routes to Chronicle staging with that provenance pinned, never to a claim;
- 6 `external` statistical rows become deterministic Chronicle staging facts in `data/ledger/be_admin_outturns.jsonl`, each carrying its underlying issuer (NBB for income tax, employee SICs, and child benefits; RVA for unemployment; EU-SILC for the Gini and AROP comparators) with JRC as the secondary publisher;
